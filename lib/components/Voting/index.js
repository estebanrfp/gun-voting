...
gun.get(`~${ pubKey }`).get('voting').once(data => {
  const proposalsData = [
    ...
    {
      title: 'SECONDLIFE using MySQL',
      desc: 'Proposal from @Second Life',
      votes: 0
    },
    {
    ...
    {
      title: 'OPENSIM using MYSQL',
      desc: 'Proposal from @Opensim',
      votes: 0
    },
    {
  ...
  document.querySelector('button#generateUrl').addEventListener('click', () => {
    ...
    gun.get(`~${ pubKey }`).get('status').put('Voting !')
  })
...
function Vot (proposals) {
  ...
  function winningProposal () {
    ...
    gun.get(`~${ pubKey }`).get('status').put(`${ title[0].title } is winning the voting!`)

    if (title[0].votes === title[1].votes) {
      gun.get(`~${ pubKey }`).get('status').put('The votes is tied!!!!')
    }
  }
  ...
  function wonProposal (title) {
    ...
    gun.get(`~${ pubKey }`).get('status').put(`${ title } won. The voting is over.`)
...
gun.get(`~${ pubKey }`).get('status').on(data => {
  winningContainer.innerHTML = data
})

CD('Aug 29, 2021 21:30:00')

// gun.get('~null').get('voting').put(null)
// gun.get('~null').get('status').put(null)
